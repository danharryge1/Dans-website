...
describe("<HeroScreen />", () => {
  ...
  it("renders a <video> on the reality side with correct poster and mp4 source", () => {
    const { container } = render(<HeroScreen {...DEFAULT_PROPS} />);
    const video = container.querySelector("[data-hero-reality] video") as HTMLVideoElement;
    expect(video).not.toBeNull();
    expect(video.getAttribute("poster")).toContain("nextup-v2-poster.jpg");
    ...
    expect(source?.getAttribute("src")).toContain("nextup-v2.mp4");
  });
  ...
  it("provides a screen-reader sentence describing the comparison", () => {
    render(<HeroScreen {...DEFAULT_PROPS} />);
    expect(
      screen.getByText(/drag the seam/i, { selector: ".sr-only" }),
    ).toBeInTheDocument();
  });
});
